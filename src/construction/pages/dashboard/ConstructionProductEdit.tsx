...
export default function ConstructionProductEdit() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      const { data: customization } = await supabase
        .from('construction_customization_options')
        .select('*')
        .eq('product_id', id)
      ...
      setFormData(prev => ({
        ...
        product: {
          ...
          size_ft: product.size_ft || '',
          bedrooms: product.bedrooms || '',
          bathrooms: product.bathrooms || '',
          area_sqm: product.area_sqm ? product.area_sqm.toString() : '',
        ...
        customization: (customization || []).reduce((acc, item) => {
          acc[item.option_type] = {
            value: item.option_value,
            priceModifier: item.price_modifier.toString()
          }
          return acc
        }, {} as any)
      }))
      ...
      // Set preview images
      const imagePreviews = (images || []).map(img => img.public_url)
    ...
    try {
      ...
      const { error: productError } = await supabase
        ...
        .update({
          ...
          size_ft: formData.product.size_ft,
          bedrooms: formData.product.bedrooms,
          bathrooms: formData.product.bathrooms,
      ...
      for (let i = 0; i < formData.images.length; i++) {
        ...
        const storagePath = `${session.user.id}/${id}/${image.name}`
        ...
        await supabase
          ...
          .insert({
            ...
            is_primary: i === 0,
            sort_order: formData.existingImages.length + i
          })
      }
      ...
      for (const [docType, file] of Object.entries(formData.documents)) {
        if (file) {
          const storagePath = `${session.user.id}/${id}/${docType}/${file.name}`
          const { error: uploadError } = await supabase.storage
      ...
      for (const [optionType, optionData] of Object.entries(formData.customization)) {
        if (optionData.value) {
          // Check if option exists
          const existing = formData.existingDocuments.find(d => d.option_type === optionType)
          if (existing) {
            await supabase
              .from('construction_customization_options')
              .update({
                option_value: optionData.value,
                price_modifier: parseFloat(optionData.priceModifier || '0')
              })
              .eq('id', existing.id)
          } else {
            await supabase
