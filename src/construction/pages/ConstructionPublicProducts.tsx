...
const FONT_IMPORT = "https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&family=Cormorant+Garamond:wght@300;400;500;600&display=swap"
...
export default function ConstructionPublicProducts() {
  ...
  useEffect(() => {
    ...
    // Inject !important override for any global gray background
    if (!document.getElementById('construction-bg-override')) {
      const s = document.createElement('style')
      s.id = 'construction-bg-override'
      s.textContent = 'html, body { background-color: #ffffff !important; background: #ffffff !important; }'
    ...
    return () => {
      document.getElementById('construction-bg-override')?.remove()
    }
  }, [])
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", color: C.text }}>
      ...
      {/* Filter bar */}
      <div style={{ backgroundColor: '#ffffff', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: '72px', zIndex: 900 }}>
      ...
      <div style={{ backgroundColor: '#ffffff', width: '100%' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '60px 32px 80px' }}>
                  ...
                  >
                    <Link to={`/construction/${product.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                      ...
                      <div style={{ padding: '24px 24px 20px', backgroundColor: '#ffffff' }}>
                        ...
                        <h3 style={{ fontSize: '20px', fontWeight: 400, margin: '0 0 8px 0', fontFamily: "'Cormorant Garamond', Georgia, serif", color: C.charcoal, lineHeight: 1.3, letterSpacing: '-0.01em' }}>
                          {product.title}
                        </h3>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div>
                            <span style={{ fontSize: '22px', fontWeight: 600, color: C.charcoal }}>${product.price_cad.toLocaleString()}</span>
                            <span style={{ fontSize: '12px', color: C.subtext, marginLeft: '5px' }}>CAD</span>
                          </div>
                          <div
                            style={{ background: C.orange, color: C.white, padding: '10px 22px', borderRadius: '100px', fontSize: '13px', fontWeight: 600, letterSpacing: '0.02em', transition: 'background 0.2s' }}
          ...
          <div style={{ marginTop: '80px', background: C.teal, borderRadius: '20px', padding: '52px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '32px', flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
            ...
            <Link to="/construction/custom" style={{ background: C.orange, color: C.white, padding: '14px 32px', borderRadius: '100px', fontSize: '15px', fontWeight: 600, textDecoration: 'none', whiteSpace: 'nowrap', letterSpacing: '0.02em', position: 'relative', zIndex: 1 }}>
              Start Custom Order
            </Link>
          </div>
        </div>
