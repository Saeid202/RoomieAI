/**
 * Secure Logger — React Frontend
 *
 * Drop-in replacement for console.log with automatic PII/financial data
 * redaction, environment-based log levels, and structured JSON output.
 *
 * Usage:
 *   import { logger } from '@/utils/logger';
 *   logger.info('User action', { userId: 'u_123' });
 */
...
function getConfiguredLevel(): LogLevel {
  const override = import.meta.env.VITE_LOG_LEVEL as string;
...
// ─── Browser Console Protection (production only) ────────────────────────────
...
if (IS_PROD && typeof window !== 'undefined') {
  ...
  console.log   = (...args: unknown[]) => _log(...args.map((a) => sanitize(a)));
  console.warn  = (...args: unknown[]) => _warn(...args.map((a) => sanitize(a)));
  console.error = (...args: unknown[]) => _error(...args.map((a) => sanitize(a)));
  console.debug = (...args: unknown[]) => _debug(...args.map((a) => sanitize(a)));
}
...
function emit(level: LogLevel, message: string, args: unknown[]): void {
  ...
  if (IS_DEV) {
    // Plain output in dev — no sanitization, full visibility
