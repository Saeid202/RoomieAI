...
export default function N4FormPage() {
    ...
    const removeRentRow = (index: number) => {
        setFormData((prev) => ({ ...prev, rentDetails: prev.rentDetails.filter((_, i) => i !== index) }));
    };

    const calculateTotals = () => {
        const total = formData.rentDetails.reduce((sum, row) => sum + (parseFloat(row.rentOwing) || 0), 0);
        const formattedTotal = formatMoney(total);
        setFormData((prev) => ({ ...prev, totalRentOwing: formattedTotal, amountOwing: formattedTotal }));
    ...
    return (
        <div className="min-h-screen w-full -mx-6 -mt-6">
            ...
            <div className="bg-gradient-to-b from-slate-100 via-slate-100 to-slate-200 px-6 py-8 print:bg-white print:p-0">
                <div id="n4-form-content" className="w-full bg-slate-50 shadow-[0_2px_24px_0_rgba(0,0,0,0.07)] border border-slate-200/60 rounded-2xl overflow-hidden print:shadow-none print:border-none print:rounded-none">
                    ...
                    <div className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200 px-8 py-6 flex justify-between items-start">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900">Notice to End your Tenancy for Non-payment of Rent</h1>
                            <p className="text-slate-500 text-sm mt-1">Form N4 (Residential Tenancies Act, 2006)</p>
                        </div>
                        <div className="bg-slate-200 text-slate-700 px-3 py-1 rounded-full text-sm font-bold">LTB - N4</div>
                    </div>
                    ...
                    <div className="px-8 py-8 space-y-10 print:p-0">
                            ...
                            <Card>
                                <CardContent className="py-4 grid md:grid-cols-2 gap-4">
                                    <div className="md:col-span-2">
                                        ...
                                        <Input value={formData.unitAddress} onChange={(e) => handleInputChange("unitAddress", e.target.value)} className="mt-1.5" />
                                    </div>
                                    <div>
                                        <Label>City/Town</Label>
                                        <Input value={formData.unitCity} onChange={(e) => handleInputChange("unitCity", e.target.value)} className="mt-1.5" />
                                    </div>
                                    <div>
                                        <Label>Province</Label>
                                        <Input value="Ontario" readOnly className="mt-1.5 bg-slate-100" />
                                    </div>
                                    <div>
                                        <Label>Postal Code</Label>
                                        <Input value={formData.unitPostal} onChange={(e) => handleInputChange("unitPostal", e.target.value)} className="mt-1.5" />
                                    </div>
                                </CardContent>
                            </Card>
                        </div>

                        {/* Section 3 — Payment & Termination */}
                        <div>
                            <SectionHeader num={3} title="Payment & Termination" />
                                        ...
                                        <div className="w-full md:w-1/2">
                                            ...
                                            <div className="relative mt-1.5">
                                                ...
                                                <Input
                                                    ...
                                                    readOnly
                                                    className="pl-8 bg-slate-100 font-bold text-lg"
                                                    placeholder="0.00"
                                                />
                                            </div>
                        ...
                        {/* Section 4 — Rent Calculation */}
                        <div>
                            <SectionHeader num={4} title="Rent Owing Calculation" />
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-end py-3 border-b">
                                ...
                                <CardContent className="pt-4">
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody className="divide-y">
                                                {formData.rentDetails.map((row, idx) => (
                                                    <tr key={idx} className="group hover:bg-slate-50">
                                                        <td className="p-2">
                                                            <Input placeholder="dd/mm/yyyy" value={row.periodFrom} onChange={(e) => updateRentRow(idx, "periodFrom", e.target.value)} />
                                                        </td>
                                                        <td className="p-2">
                                                            <Input placeholder="dd/mm/yyyy" value={row.periodTo} onChange={(e) => updateRentRow(idx, "periodTo", e.target.value)} />
                                                        </td>
                                                        <td className="p-2">
                                                            <Input type="number" placeholder="0.00" value={row.rentCharged} onChange={(e) => updateRentRow(idx, "rentCharged", e.target.value)} />
                                                        </td>
                                                        <td className="p-2">
                                                            <Input type="number" placeholder="0.00" value={row.rentPaid} onChange={(e) => updateRentRow(idx, "rentPaid", e.target.value)} />
                                                        </td>
                                                        <td className="p-2">
                                                            <Input type="number" placeholder="0.00" value={row.rentOwing} onChange={(e) => updateRentRow(idx, "rentOwing", e.target.value)} className="font-semibold bg-red-50 text-red-700" />
                                                        </td>
                                            ...
                                            <tfoot className="bg-slate-100 font-semibold border-t">
                                                <tr>
                        ...
                        {/* Section 5 — Signature & Delivery */}
                        <div>
                            <SectionHeader num={5} title="Signature & Delivery" />
                            <div className="grid md:grid-cols-2 gap-6">
                                <Card>
                            ...
                            <Card>
                                <CardContent className="py-4 grid md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <Input value={formData.repName} onChange={(e) => handleInputChange("repName", e.target.value)} className="mt-1.5" />
                                    </div>
                                    <div>
                                        <Label>LSUC #</Label>
                                        <Input value={formData.repLsuc} onChange={(e) => handleInputChange("repLsuc", e.target.value)} className="mt-1.5" />
                                    </div>
                                    <div>
                                        <Label>Company Name</Label>
                                        <Input value={formData.repCompany} onChange={(e) => handleInputChange("repCompany", e.target.value)} className="mt-1.5" />
                                    </div>
                                    <div>
                                        <Label>Phone</Label>
                                        <Input value={formData.repPhone} onChange={(e) => handleInputChange("repPhone", e.target.value)} className="mt-1.5" />
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
