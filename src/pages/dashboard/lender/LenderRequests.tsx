      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="border-2 border-purple-200 bg-gradient-to-br from-purple-500/20 to-indigo-500/20">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-black text-gray-900">{requests.filter(r => r.lender_access_consent).length}</p>
            ...
            <div>
              <p className="text-2xl font-black text-gray-900">
                {requests.filter(r => {
                  ...
                  return (Date.now() - new Date(r.consent_granted_at).getTime()) < 7 * 24 * 60 * 60 * 1000;
                }).length}
              </p>
          ...
          <Card className="w-full max-w-2xl max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-black bg-gradient-to-r from-pink-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent">Borrower Profile</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedRequest.full_name && (
                  <div className="flex items-center gap-2 p-3 bg-pink-50 rounded-lg">
                    ...
                    <div><p className="text-xs text-gray-500">Name</p><p className="font-semibold">{selectedRequest.full_name}</p></div>
                  </div>
                )}
                {selectedRequest.email && (
                  <div className="flex items-center gap-2 p-3 bg-purple-50 rounded-lg">
                    <Mail className="h-4 w-4 text-purple-600" />
                    <div><p className="text-xs text-gray-500">Email</p><p className="font-semibold">{selectedRequest.email}</p></div>
                  </div>
                )}
                {selectedRequest.phone_number && (
                  <div className="flex items-center gap-2 p-3 bg-indigo-50 rounded-lg">
                    <Phone className="h-4 w-4 text-indigo-600" />
                    <div><p className="text-xs text-gray-500">Phone</p><p className="font-semibold">{selectedRequest.phone_number}</p></div>
                  </div>
                )}
                {selectedRequest.annual_income && (
                  <div className="flex items-center gap-2 p-3 bg-pink-50 rounded-lg">
                    <DollarSign className="h-4 w-4 text-pink-600" />
                    <div><p className="text-xs text-gray-500">Annual Income</p><p className="font-semibold">${selectedRequest.annual_income.toLocaleString()}</p></div>
                  </div>
                )}
                {selectedRequest.purchase_price_range && (
                  <div className="p-3 bg-purple-50 rounded-lg">
                    <p className="text-xs text-gray-500">Purchase Price Range</p>
                    <p className="font-semibold">{selectedRequest.purchase_price_range}</p>
                  </div>
                )}
                {selectedRequest.credit_score_range && (
                  <div className="p-3 bg-indigo-50 rounded-lg">
                    <p className="text-xs text-gray-500">Credit Score Range</p>
                    <p className="font-semibold">{selectedRequest.credit_score_range}</p>
                  </div>
                )}
                {selectedRequest.down_payment_amount && (
                  <div className="p-3 bg-pink-50 rounded-lg">
                    <p className="text-xs text-gray-500">Down Payment</p>
                    <p className="font-semibold">${selectedRequest.down_payment_amount.toLocaleString()}</p>
                  </div>
                )}
                {selectedRequest.employment_status && (
                  <div className="p-3 bg-purple-50 rounded-lg">
                    <p className="text-xs text-gray-500">Employment Status</p>
                    <p className="font-semibold">{selectedRequest.employment_status}</p>
                  </div>
                )}
                {selectedRequest.mortgage_purpose && (
                  <div className="p-3 bg-indigo-50 rounded-lg">
                    <p className="text-xs text-gray-500">Mortgage Purpose</p>
                    <p className="font-semibold">{selectedRequest.mortgage_purpose}</p>
                  </div>
                )}
                {selectedRequest.desired_location && (
                  <div className="p-3 bg-pink-50 rounded-lg">
                    <p className="text-xs text-gray-500">Desired Location</p>
                    <p className="font-semibold">{selectedRequest.desired_location}</p>
                  </div>
                )}
              </div>
            </CardContent>
      ...
      {requests.length === 0 ? (
        <Card className="border-2 border-dashed border-purple-200 bg-gradient-to-br from-pink-500/10 to-purple-500/10">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">Borrowers who grant lender access will appear here.</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-gray-900 text-sm">{request.full_name || "Anonymous Borrower"}</span>
                  </div>
