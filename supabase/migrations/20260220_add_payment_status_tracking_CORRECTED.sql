-- Payment Status Tracking Enhancement (CORRECTED)
-- Phase 1: Add enhanced status tracking for PAD payments
-- Created: 2026-02-19
-- CORRECTED: Uses rental_payments table (not rent_payments)
...
AND column_name IN ('failure_reason', 'failure_code', 'retry_count', 'payment_metadata')
ORDER BY column_name;

-- Check status constraint
SELECT constraint_name, check_clause
FROM information_schema.check_constraints
WHERE constraint_name = 'rental_payments_status_check';

-- Check indexes
SELECT indexname 
FROM pg_indexes 
WHERE tablename = 'rental_payments'
AND indexname LIKE '%failure%' OR indexname LIKE '%retry%' OR indexname LIKE '%pending%';
*/
