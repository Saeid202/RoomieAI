-- Fix Foreign Key Constraint Issue for Property Deletion
-- The property has related lease_contracts that need to be deleted first
...
BEGIN
    ...
    DELETE FROM property_images WHERE property_id = property_id_to_delete;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    result_message := result_message || 'Deleted ' || deleted_count || ' property images. ';
    
    -- Delete property views
    DELETE FROM property_views WHERE property_id = property_id_to_delete;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    result_message := result_message || 'Deleted ' || deleted_count || ' property views. ';
