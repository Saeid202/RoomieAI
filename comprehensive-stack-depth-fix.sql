-- COMPREHENSIVE FIX: Stack Depth Limit Exceeded Error
-- This addresses ALL potential recursion points in messaging
...
-- 2. Create ULTRA-SIMPLE policies with NO recursion
...
-- 3. Drop any problematic helper functions that might cause recursion
DROP FUNCTION IF EXISTS is_conversation_participant(UUID, UUID);
DROP FUNCTION IF EXISTS get_conversation_participants(UUID);

-- 4. Test the fix
SELECT 'Comprehensive stack depth fix applied' as status;
